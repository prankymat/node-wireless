...
Wireless.prototype.COMMANDS = {
  ...
  status: 'nmcli -t -f DEVICE,STATE d | awk -F\":INTERFACE:" \'{print $2}\'',
  essid: 'iwconfig :INTERFACE 2> /dev/null | awk -F\\\" \'{print $2}\'',
  bssid: 'iwconfig :INTERFACE 2> /dev/null | awk -F\"Access Point: \" \'{print $2}\'',
...
function wrapEchoE(command) {
  return "`which echo` -e $(" + command + ")";
}

function wrapEchoEecho(str) {
  return wrapEchoE("echo '"+str+"'");
}

function escapeHexStr(str) {
  return sh.exec(wrapEchoEecho(str.trim()),{silent: true}).output;
}

Wireless.prototype.status = function(callback) {
  var self = this;

  this.emit('command', this.commands.status);

  exec(this.commands.status, function(err, stdout, stderr) {
      if (err) {
        self.emit('error', "There was an unknown error when checking the status" + err);
...
Wireless.prototype.essid = function(callback) {
  var self = this;

  this.emit('command', this.commands.essid);

  exec(this.commands.essid, function(err, stdout, stderr) {
      if (err) {
        self.emit('error', "There was an unknown error when obtaining the essid" + err);
        callback && callback(err);
      }

      stdout = escapeHexStr(stdout);
...
Wireless.prototype.bssid = function(callback) {
  var self = this;

  this.emit('command', this.commands.bssid);

  exec(this.commands.bssid, function(err, stdout, stderr) {
      if (err) {
        self.emit('error', "There was an unknown error when obtaining the bssid" + err);
        callback && callback(err);
      }
      callback && callback(stdout.trim());
  });
...
Wireless.prototype._translate = function(string, data) {
  for (var index in data) {
    ...
    string = string.replace(re, data[index]);
  }
...
Wireless.prototype.join = function(network, password, callback) {
  if (network.encryption_wep) {
    ...
    console.log("calling wpa")
    this._executeConnectWPA(network.ssid, network.address, password, callback);
  } else {
...
Wireless.prototype._parseScan = function(scanResults) {
  ...
  _.each(lines, function(line) {
    ...
    if (line.indexOf('Cell') === 0) {
      ...
      network.ssid = escapeHexStr(network.ssid).trim();
...
Wireless.prototype._executeConnectWPA = function(essid, bssid, password, callback) {
  var self = this;
